...
Defines the structured, content-addressed element-graph model that is the source of truth for slide content and revisions, replacing per-revision HTML storage with a generic, versioned element/animation registry, copy-on-write persistence, and on-demand rendering.
