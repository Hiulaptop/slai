# SlaI — Canonical Frontend Style Reference

> Adoption note: this reference originated from the “Steep” editorial design study and is now the approved visual direction for the SlaI web application. Product naming, content, and workflows must remain SlaI-specific.

## Implemented brand baseline

- Display and wordmark fallback: Source Serif 4 via `next/font/google`
- UI and body fallback: Inter via `next/font/google`
- CSS font tokens: `--font-signifier` and `--font-sohne`
- In-app identity: dark square “S” mark with serif “SlaI” wordmark
- Browser identity: the same dark-square “S” mark, without the wordmark
- Canonical frontend token implementation: `app/globals.css`

The implemented font, color, type-scale, spacing, radius, and shadow tokens are the baseline for future frontend work. Changes require explicit product-owner approval and corresponding updates to this document.

The product owner approved consolidating the former separate “SI” browser monogram into the in-app “S” mark on 2026-07-28. `icon.svg` is current; `favicon.ico` remains a compatibility asset that must be regenerated to match.

## Original design characterization
